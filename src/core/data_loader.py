...
class DataLoader:
    ...
    def _load_csv(self, file_path: Path, use_efficient_loading: bool) -> pd.DataFrame:
        ...
        if use_efficient_loading:
            ...
            # 청크 단위로 로드하고 결합
            chunks = []
            logger.info(f"대용량 CSV 파일 로드 중: {file_path.name} (청크 단위 로딩)")
            
            # progress_bar = tqdm(total=file_path.stat().st_size, unit='B', unit_scale=True)
            for chunk in pd.read_csv(file_path, dtype=dtypes, chunksize=50000):
                chunks.append(chunk)
                # progress_bar.update(chunk.memory_usage(deep=True).sum())
    ...
    def _load_excel(self, file_path: Path, sheet_name: Union[str, int], use_efficient_loading: bool) -> pd.DataFrame:
        ...
        # 선택한 시트 로드
        logger.info(f"엑셀 파일 로드 중: {file_path.name}, 시트: {sheet_to_load}")
    ...
    def _optimize_dataframe(self) -> None:
        ...
        # 최적화 시작
        logger.info(f"데이터프레임 메모리 최적화 시작 (현재 사용량: {mem_usage_before:.2f} MB)")
        
        # 열 데이터 타입 최적화
        for col in self.df.columns:
            # 결측값이 있으면 Int64/Float64 같은 nullable 타입 사용 고려
            ...
            # 문자열 열
                ...
                if not self.df[col].isna().all():  # 모든 값이 NaN이 아닌 경우만
                    unique_count = self.df[col].nunique()
                    unique_ratio = unique_count / len(self.df[col])
                    
                    # 고유 값 비율이 낮거나 고유값이 적은 경우 category 타입으로 변환
        ...
        logger.info(f"데이터프레임 최적화 완료: {mem_usage_before:.2f}MB → {mem_usage_after:.2f}MB ({memory_saved:.2f}MB 절약, {self.file_info['memory_reduction_percent']}% 감소)")
